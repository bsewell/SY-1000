...
QString MidiTable::getRoot(QString root)
{
    if(root=="10"){root="Structure";}
    if(root=="00"){root="System";};
    if(root=="Tables"){root="Tables";};
    //else{root = "Structure";};
    return root;
}

/* BUG FIX (contributed): All getMidiMap() overloads now guard indexOf() against -1.
 * Previously, any unrecognized hex parameter (e.g. SY-1000 addresses not present
 * in the VG-99 XML midi map) caused indexOf() to return -1, and .at(-1) triggered
 * an immediate out-of-bounds crash. Returns an empty Midi struct on lookup failure. */

Midi MidiTable::getMidiMap(QString root)
{
    int idx = midiMap.id.indexOf(this->getRoot(root));
    if(idx == -1) { Midi empty; return empty; }
    Midi section = midiMap.level.at( idx );
    return section;
}

Midi MidiTable::getMidiMap(QString root, QString hex1)
{
    int idx0 = midiMap.id.indexOf(this->getRoot(root));
    if(idx0 == -1) { Midi empty; return empty; }
    Midi section = midiMap.level.at( idx0 );
    int idx1 = section.id.indexOf(hex1);
    if(idx1 == -1) { Midi empty; return empty; }
    Midi level1 = section.level.at( idx1 );
    return level1;
}
...
Midi MidiTable::getMidiMap(QString root, QString hex1, QString hex2)
{
    ...
    if(idx1 == -1) { Midi empty; return empty; }
    Midi level1 = section.level.at( idx1 );
    int idx2 = level1.id.indexOf(hex2);
    if(idx2 == -1) { Midi empty; return empty; }
    Midi level2 = level1.level.at( idx2 );
    return level2;
}
...
Midi MidiTable::getMidiMap(QString root, QString hex1, QString hex2, QString hex3)
{
    ...
    {
        int idx3 = level2.id.indexOf(hex3);
        if(idx3 == -1) { Midi empty; return empty; }
        level3 = level2.level.at( idx3 );
    };
    return level3;
}
...
Midi MidiTable::getMidiMap(QString root, QString hex1, QString hex2, QString hex3, QString hex4)
{
    ...
    int idx4 = level3.id.indexOf(hex4);
    if(idx4 == -1) { Midi empty; return empty; }
    Midi level4 = level3.level.at( idx4 );
    return level4;
}
...
Midi MidiTable::getMidiMap(QString root, QString hex1, QString hex2, QString hex3,  QString hex4, QString hex5)
{
    ...
    int idx3 = level2.id.indexOf(hex3);
    if(idx3 == -1) { Midi empty; return empty; }
    Midi level3 = level2.level.at( idx3 );
    int idx4 = level3.id.indexOf(hex4);
    if(idx4 == -1) { Midi empty; return empty; }
    Midi level4 = level3.level.at( idx4 );
    int idx5 = level4.id.indexOf(hex5);
    if(idx5 == -1) { Midi empty; return empty; }
    Midi level5 = level4.level.at( idx5 );
    return level5;
}
...
int MidiTable::getRange(QString root, QString hex1, QString hex2, QString hex3)
{
    ...
    if(range.level.isEmpty()) { return 0; }
    int lastIndex;
...
bool MidiTable::isData(QString root, QString hex1, QString hex2, QString hex3)
{
    ...
    /* When FX has alot off settings (more than ) it's spanned over more then one entry in the
        midi.xml, so when out of range we jump to the next entry and start from 00. */
    // bool ok;
    //if(root.toInt(&ok, 16)<10){root = "System";} else {root = "Structure";};
    range = getMidiMap(this->getRoot(root), hex1, hex2, hex3);
    if(range.level.isEmpty()) { return false; }
...
{
    Midi range = getMidiMap(this->getRoot(root), hex1, hex2, hex3);

    if(range.level.isEmpty()) { return QString(); }  // BUG FIX: guard against empty getMidiMap result
    ...
    if(range.level.last().type.contains("DATA"))
    {
        ...
        int maxRange = 128;
        if (root == "Tables" && (hex3 == "00" || hex3 == "02")) { maxRange = 256; }; // if Assign target, change range.
        ...
        int dataIdx = range.id.indexOf(index1);
        if(dataIdx < 0 || dataIdx >= range.level.size()) { return QString(); }
        Midi dataRange = range.level.at(dataIdx);
    ...
    if(!range.id.contains(hex) && range.id.contains("range"))
    {
        ...
        while(range.id.indexOf("range", i) != -1)
        {
            ...
            if(valueInt == start || (valueInt > start && valueInt < end) || valueInt == end)
            {
                ...
                if(result>0 && min<0){valueStr.prepend("+");};
                break;
            };
            i++;
        };
...
QString MidiTable::getHeader(bool receive)
{
    ...
    QString header("F041100000006912");
    if(receive==true){header = "F041100000006911"; };
...
QString MidiTable::dataChange(QString hex0, QString hex1, QString hex2, QString hex3, QString hex4)
{
    QString sysxMsg;
...
QString MidiTable::dataChange(QString hex0, QString hex1, QString hex2, QString hex3, QString hex4, QString hex5)
{
    QString sysxMsg;
    sysxMsg.append(getHeader(false));
    sysxMsg.append(hex0);
...
QString MidiTable::nameRequest(int bank, int patch)
{
    ...
    bool ok;
    if(preferences->getPreferences("Window", "BassMode", "bool")=="true"){base = 40; };

    QString addr1, addr2;
    if((bank != 0 && patch != 0) && bank <= bankTotalAll)
    {
        int patchOffset = ((((bank - 1 ) * patchPerBank) + patch)*2) - 2;
        int addrMaxSize = 128;
        int n = int(patchOffset / addrMaxSize);
        addr1 = QString::number(base + n, 16).toUpper();  // guitar = 32, bass = 40
    ...
    else
    {
        addr1 = tempDataWrite;
        addr2 = "00";
        if(preferences->getPreferences("Window", "BassMode", "bool")=="true"){addr2 = "02";};
    };
    QString hex2 = "00";
    QString hex3 = "00";
