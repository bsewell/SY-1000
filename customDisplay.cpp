...
void customDisplay::paintEvent(QPaintEvent *)
{
   ...
   /* const double ratio = preferences->getPreferences("Window", "Font", "ratio").toDouble(&ok);
    ...
    while(this->mainLabel->geometry().width() < QFontMetrics(this->mainLabel->font()).horizontalAdvance(this->mainLabel->text()))
    {
        Preferences *preferences = Preferences::Instance();
        bool ok;
        const double ratio = preferences->getPreferences("Window", "Font", "ratio").toDouble(&ok);
...
void customDisplay::setFrameVisible(bool visible)
{
