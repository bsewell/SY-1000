    ...
    : QWidget(parent)
{
    ...
    this->image = QPixmap(imagePathPanelBar).scaled(17*ratio, 640*ratio, Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
{ 
	if (event->button() == Qt::LeftButton) this->dragStartPosition = event->position().toPoint(); 
...
void floorPanelBar::mouseMoveEvent(QMouseEvent *event)
{
    if ( event->buttons() == Qt::LeftButton && 
		(event->position().toPoint() - dragStartPosition).manhattanLength() > QApplication::startDragDistance() )
	{
		QPoint mousePoint = event->position().toPoint();
		emit showDragBar( QPoint(this->x() + mousePoint.x(), this->y()) );
	};
