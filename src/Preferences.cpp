/****************************************************************************
**
** Copyright (C) 2007~2023 Colin Willcocks.
...
// BUG FIX: Use writable app data directory instead of bare relative path.
// When launched from Finder, CWD is "/" (read-only), causing preferences.xml
// writes to fail silently and the app to loop with QPainter errors.
static QString preferencesFilePath()
{
    QString dir = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    QDir().mkpath(dir);  // create ~/Library/Application Support/SY-1000FloorBoard/ if needed
    return dir + "/preferences.xml";
}
...
void Preferences::loadPreferences(QString fileName)
{
    ...
    //qDebug() << fileName << "file exists =" << file.exists();
...
void Preferences::savePreferences()
{
    ...
    QFile file(preferencesFilePath());  // BUG FIX: use writable app data path
    ...
    xout.writeComment("Preferences of the SY-1000 FloorBoard application.");
    ...
    std::sort(sortIndexList.begin(), sortIndexList.end());
