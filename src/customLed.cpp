...
customLed::customLed(bool active, double ledRatio, QWidget *parent,
					 QString imagePath)
    : QWidget(parent)
{
	this->active = active;
    ...
    this->ledRatio = ledRatio;

	setOffset(0);
    setGeometry(0, 0, ledSize.width(), ledSize.height());
}
...
void customLed::paintEvent(QPaintEvent *)
{
    ...
    QRectF target(0.0 , 0.0, ledSize.width()*ledRatio*ratio, ledSize.height()*ledRatio*ratio);
