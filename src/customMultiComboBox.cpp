...
void customMultiComboBox::makeList()
{
    ...
    display_11 = new customControlMidiTable(this, hex1, hex2 ,hex3, "SY-1000 Patch", comboList);

    QVBoxLayout *oneLayout = new QVBoxLayout;
    //oneLayout->setContentsMargins(0, 0, 0, 0);
...
{
    // when SY-1000 patch combo is changed
...
void customMultiComboBox::displayUpdate()
{
    ...
    bool ok;
    QList <QString> items;
    items = getMPTitems();  // get the data block from the SY-1000 system MPT area
...
QList<QString> customMultiComboBox::getMPTitems()
{
    ...
    SysxIO *sysxIO = SysxIO::Instance();
    bool ok;
    emit sysxIO->setStatusProgress(100);
    if (sysxIO->isConnected() && sysxIO->deviceReady())
    {
        emit sysxIO->setStatusSymbol(2);
        emit sysxIO->setStatusMessage(tr("Request MPT data"));
        ...
        sysxIO->sendSysx(MPTrequest, 2000); // SY-1000 System MPT bank data Request.
        emit sysxIO->setStatusProgress(0);
    };
    return MPT_items;
}
...
QList<QString> customMultiComboBox::MPTReply(QString replyMsg)
{
    ...
    sysxIO->setDeviceReady(true); // Free the device after finishing interaction.
        if(sysxIO->noError() && replyMsg.size()/2 == 397)  // data format from the SY-1000 MPT
        {
