    ...
    : QWidget(parent)
{
    ...
    bool ok;
    this->ratio = preferences->getPreferences("Window", "Scale", "ratio").toDouble(&ok);
    ...
    this->slideSize = QPixmap(this->slideImagePath).size()*this->ratio;
    this->sliderButtonSize = QPixmap(this->sliderButtonImagePath).size()*this->ratio;
...
void customSlider::paintEvent(QPaintEvent *)
{
    ...
    double x = 0.0;
    if(this->sliderButtonImagePath.contains("geq_slider_knob")){x=8.0*this->ratio;};
    QRectF target(x , yOffset*ratio, sliderButtonSize.width()*this->ratio, sliderButtonSize.height()*this->ratio);
    ...
    painterBuffer.setRenderHint(QPainter::SmoothPixmapTransform, true);
...
void customSlider::setOffset(double _newValue)
{
    ...
    double range = ((slideSize.height()/ratio)  - (sliderButtonSize.height()/ratio));
...
void customSlider::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        setFocus();
        mouseTrigger(event->position().toPoint());
        emitValue(value);
    };
}
