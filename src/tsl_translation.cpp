...
{
    QString m = "xpos = " + QString::number(pos, 10)+ "   value = " + QString(value.toHex().toUpper());
...
QByteArray tsl_translation::get_chain(int value)
{
    ...
    QByteArray subout = QByteArrayLiteral("\x13\x14");
    QByteArray retmix = QByteArrayLiteral("\x10\x0E\x08\x0F");
    QByteArray farend = QByteArrayLiteral("\x11\x12\x15\x16\x17");
    QByteArray chain;
...
QByteArray tsl_translation::convert_Assign(const char* Json_name)
{
    MidiTable *midiTable = MidiTable::Instance();
    bool ok;
    int assign = QString::number(GetJsonHex(Json_name, a).at(0), 16).toInt(&ok, 16);
    ...
    QString assign_after = items.desc;
    ...
    QString m = "Assign before = "+assign_before + "  Assign after = " + assign_after + " Assign actual = " + index;
    std::string st = m.toStdString();
    std::cerr << st << std::endl;
...
QByteArray tsl_translation::convertFromTSL(int patch)
{
    ...
    if(device.contains("SY-300"))
    {
        ...
        /* chain items:
          ...
          00=cmp 01=ds 02=amp 08=eq2 03=ns 09=cho 0F=fv2 0B=dd2 18=bal3 0C=dd3 0D=rev 11=div 12=mix
          13=sub/l 14=sub/r 15=main/l 16=main/r 17=mstr
     ...
     /* patchOscCommonOSCRing2	0
        ...
        if(bass_mode==true){x = 3626;} else {x = 3758;}; // start of FX1
       ...
       // };
        ...
        std::string st = Fx1type.toStdString();
        std::cerr << st << std::endl;


        if(bass_mode==true){x = 4502;} else {x = 4634;}; // start of Fx2
        ...
        std::string st2 = Fx2type.toStdString();
        std::cerr << st2 << std::endl;


        if(bass_mode==true){x = 5378;} else {x = 5510;}; // start of Fx3
        ...
        std::string st3 = Fx3type.toStdString();
        std::cerr << st3 << std::endl;
        ...
        if(Fx4type.contains("00")){
            if(bass_mode==true){x = 3687;} else {x = 3719;}; // start of chorus block
            SY1K_hex_replace(x, 1, t);  // switch on chorus block
        ...
        */
        std::string st4 = Fx4type.toStdString();
        std::cerr << st4 << std::endl;
        ...
        }else
        {
            ...
            QString m = "SHIT HAPPENED IN SY-300 FILE TRANSLATION";
            std::string st = m.toStdString();
            std::cerr << st << std::endl;
